...
function normalizeAlbum(value: unknown): AlbumPublik | null {
  if (!value || typeof value !== 'object' || Array.isArray(value)) {
    return null;
  }

  const row = value as AlbumRow;
  ...
  return {
    ...
    slug,
    deskripsi:
      safeString(row.deskripsi) ||
      'Dokumentasi kegiatan dan perkembangan Desa Keji.',
    kategori: safeString(row.kategori) || 'Dokumentasi Desa',
    tanggalKegiatan: nullableString(row.tanggal_kegiatan),
    lokasi: safeString(row.lokasi) || 'Desa Keji',
    fotoSampulUrl: nullableString(row.foto_sampul_url),
  };
}
...
function normalizeFoto(
  ...
  const row = value as FotoRow;

  const id = safeString(row.id);
  const urlFoto = safeString(row.url_foto);
  ...
  return {
    ...
    altText:
      safeString(row.alt_text) ||
      `Dokumentasi ${judulAlbum}`,
    urutan: safeInteger(row.urutan),
  };
}
...
export async function generateMetadata({
  ...
  if (!normalizedSlug) {
    return {
      title: 'Album Galeri Desa Keji | SIJI',
      description: 'Dokumentasi kegiatan Desa Keji.',
    };
  }
  ...
  const judul = safeString(data?.judul);
  const deskripsi = safeString(data?.deskripsi);

  return {
    title: judul
      ? `${judul} | Galeri Desa Keji`
      : 'Album Galeri Desa Keji | SIJI',

    description:
      deskripsi ||
      'Dokumentasi kegiatan pemerintahan, masyarakat, budaya, dan pembangunan Desa Keji.',
  };
}
...
export default async function DetailAlbumPage({
  ...
  const [albumResult, layananResult] = await Promise.all([
    supabaseAdmin
      .from('album_galeri')
      .select(`
        id,
  ...
  if (albumResult.error) {
    console.error('Gagal mengambil detail album:', {
      message: albumResult.error.message,
      code: albumResult.error.code,
      details: albumResult.error.details,
      hint: albumResult.error.hint,
    });

    notFound();
  }

  const album = normalizeAlbum(albumResult.data);
  ...
  const fotoResult = await supabaseAdmin
    .from('foto_galeri')
    .select(`
      id,
      url_foto,
  ...
  if (fotoResult.error) {
    console.error('Gagal mengambil foto album:', {
      message: fotoResult.error.message,
      code: fotoResult.error.code,
      details: fotoResult.error.details,
      hint: fotoResult.error.hint,
    });
  }

  const daftarFoto = (fotoResult.data ?? [])
    .map((foto) => normalizeFoto(foto, album.judul))
    .filter((foto): foto is FotoPublik => foto !== null);
  ...
  const daftarLayanan: PilihanLayanan[] = (
    ...
    .map((layanan) => {
      ...
      const layananSlug = safeString(layanan.slug);
          ...
          <div className="relative min-h-[340px] overflow-hidden sm:min-h-[400px]">
            ...
            <div className="relative flex min-h-[340px] max-w-4xl flex-col justify-end px-6 py-8 sm:min-h-[400px] sm:px-8 sm:py-10">
              ...
              <div className="mt-6 flex flex-wrap gap-3">
                <HeroBadge icon={CalendarDays}>
                  {formatTanggal(album.tanggalKegiatan)}
                </HeroBadge>

                <HeroBadge icon={MapPin}>
                  {album.lokasi}
                </HeroBadge>

                <HeroBadge icon={Images}>
                  {formatAngka(daftarFoto.length)} foto
                </HeroBadge>
              </div>
            </div>
          </div>
              ...
              <div className="mb-6 flex items-start gap-4">
                ...
                <div>
                  ...
                  <p className="mt-2 text-sm font-medium leading-7 text-slate-500">
                    Klik gambar untuk membuka foto dalam ukuran penuh.
                  </p>
                </div>
              </div>
  ...
  return (
    <div className="rounded-3xl border border-dashed border-emerald-200 bg-white px-6 py-16 text-center shadow-sm">
      ...
      <p className="mx-auto mt-2 max-w-md text-sm font-medium leading-6 text-slate-500">
        Foto dokumentasi akan tampil setelah ditambahkan dan
        dipublikasikan melalui halaman administrator.
      </p>
    </div>
  );
}