// components/public/KategoriUmurCharts.tsx
...
export interface StatistikKategoriUmur {
  key: string;
  label: string;
  rentang: string;
  jumlah: number;
...
interface KategoriUmurChartsProps {
  data: StatistikKategoriUmur[];
...
export default function KategoriUmurCharts({
  data,
  totalPenduduk,
  tahunData,
}: KategoriUmurChartsProps) {
  const dataAktif =
    useMemo(
      () =>
        data.filter(
  ...
  function downloadCsv() {
    ...
    link.download =
      `kategori-umur-desa-keji-${tahunData}.csv`;
      ...
      <div className="flex flex-col gap-4 border-b border-slate-100 px-5 py-5 sm:px-6 lg:flex-row lg:items-center lg:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm font-medium text-slate-500">
            Komposisi kategori umur
            penduduk aktif Desa Keji
            tahun {tahunData}.
          </p>
        </div>
